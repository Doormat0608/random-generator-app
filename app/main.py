...
if page == "О разработчике":
    ...
    st.sidebar.header("Первое линейное распределение f(x)=a1*x+b1")
    a1 = st.sidebar.number_input("a1", value=config_data.get('a1', 1.0))
    b1 = st.sidebar.number_input("b1", value=config_data.get('b1', 0.0))

    st.sidebar.header("Второе линейное распределение f(x)=a2*x+b2")
    a2 = st.sidebar.number_input("a2", value=config_data.get('a2', -1.0))
    b2 = st.sidebar.number_input("b2", value=config_data.get('b2', 1.0))

    st.sidebar.header("Общие настройки")
    xmin = st.sidebar.number_input("xmin", value=config_data.get('xmin', 0.0))
    xmax = st.sidebar.number_input("xmax", value=config_data.get('xmax', 1.0))
    w = st.sidebar.slider("Вес первого распределения (w)", 0.0, 1.0, config_data.get('w', 0.5))
    N = st.sidebar.number_input("Количество генерируемых чисел N", min_value=1, max_value=1000000, value=int(config_data.get('N', 1000)), step=1)
